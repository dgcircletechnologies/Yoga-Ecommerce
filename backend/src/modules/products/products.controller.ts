...
export class ProductsController {
  ...
  @Post() @ApiBearerAuth() async create(@CurrentUser() user: AuthenticatedUser, @Body() dto: CreateProductDto) { this.requireAdmin(user); return { success: true, data: await this.productsService.create(dto) }; }
  ...
  @Patch(':id') @ApiBearerAuth() async update(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string, @Body() dto: UpdateProductDto) { this.requireAdmin(user); return { success: true, data: await this.productsService.update(id, dto) }; }
