...
export class CategoriesController {
  ...
  @Post() @ApiBearerAuth()
  async create(@CurrentUser() user: AuthenticatedUser, @Body() dto: CreateCategoryDto) { this.requireAdmin(user); return { success: true, data: await this.categoriesService.create(dto) }; }
  @Patch(':id') @ApiBearerAuth()
  async update(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string, @Body() dto: UpdateCategoryDto) { this.requireAdmin(user); return { success: true, data: await this.categoriesService.update(id, dto) }; }
