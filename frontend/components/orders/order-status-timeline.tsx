...
const stages: OrderStatus[] = ["New", "Confirmed", "Processing", "Completed"];
...
export function OrderStatusTimeline({ status }: { status: OrderStatus }) {
  ...
  return <div className="grid grid-cols-4 gap-2">{stages.map((stage, index) => <div className="text-center" key={stage}><div className={`mx-auto h-3 w-3 rounded-full ${index <= currentIndex ? "bg-brand-purple" : "bg-black/10"}`} /><p className={`mt-2 text-[9px] font-semibold uppercase tracking-[0.12em] ${index <= currentIndex ? "text-brand-purple" : "text-brand-gray"}`}>{stage}</p></div>)}</div>;
}
