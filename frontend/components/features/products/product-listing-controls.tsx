...
type ProductListingControlsProps = { categories: string[] };

export function ProductListingControls({ categories }: ProductListingControlsProps) {
  ...
  return (
    <>
      ...
      <aside aria-label="Product filters" aria-modal="true" className={`fixed inset-y-0 left-0 z-40 h-screen w-[90vw] max-w-[360px] overflow-y-auto bg-white px-6 py-6 text-brand-dark shadow-brand transition-transform duration-500 ease-in-out sm:px-8 ${isFilterOpen ? "translate-x-0" : "-translate-x-full"}`} id="product-filter" role="dialog">
