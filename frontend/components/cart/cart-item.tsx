...
export function CartItem({ item, onDecrease, onIncrease, onQuantityChange, onRemove }: CartItemProps) {
  ...
  return (
    <article className="grid gap-5 border-b border-black/10 py-7 sm:grid-cols-[120px_1fr] lg:grid-cols-[120px_1fr_130px_130px] lg:items-center lg:gap-7">
      ...
      <div className="flex items-center justify-between gap-5 text-sm lg:block"><span className="text-brand-gray lg:hidden">Price</span><span className="font-semibold text-brand-dark">{formatPrice(item.price)}</span></div>
      ...
      <div className="flex items-center justify-between border-t border-black/10 pt-4 text-sm sm:col-start-2 lg:col-auto lg:block lg:border-0 lg:pt-0"><span className="text-brand-gray lg:hidden">Subtotal</span><span className="font-semibold text-brand-dark">{formatPrice(item.price * item.quantity)}</span></div>
    </article>
  );
}
