...
export const categories: Category[] = categoryNames.map((name, index) => ({ id: `cat-${String(index + 1).padStart(3, "0")}`, name, description: `${name} thoughtfully chosen for a steady, supported practice.`, productCount: (index * 3 + 4) % 28 + 2, createdAt: new Date(Date.UTC(2026, 0, 5 + index * 4)).toISOString().slice(0, 10) }));
