...
export type TrainerInput = { name: string; email: string; password?: string; phone?: string; address?: string; profileUrl?: string; aboutMe?: string; experience?: string; specialty?: string; imageFile?: File; removeImage?: boolean };
function toFormData(data: TrainerInput) { const form = new FormData(); form.append("name", data.name); form.append("email", data.email); if (data.password) form.append("password", data.password); if (data.phone) form.append("phone", data.phone); if (data.address) form.append("address", data.address); if (data.profileUrl) form.append("profileUrl", data.profileUrl); if (data.aboutMe) form.append("aboutMe", data.aboutMe); if (data.experience) form.append("experience", data.experience); if (data.specialty) form.append("specialty", data.specialty); if (data.imageFile) form.append("image", data.imageFile); if (data.removeImage) form.append("removeImage", "true"); return form; }
