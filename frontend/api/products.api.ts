...
function mapPublicProduct(item: ApiProduct): PublicProduct { return { id: item.id, slug: item.slug, name: item.name, category: item.category?.name ?? "Uncategorized", description: item.description ?? "", price: `$${Number(item.price).toFixed(2)}`, image: item.imageUrl ?? "", stock: item.stock > 0 ? (item.stock <= 10 ? `Only ${item.stock} left` : "In stock") : "Out of stock", details: item.description ?? "", badge: item.stock <= 10 && item.stock > 0 ? "Limited" : undefined }; }
