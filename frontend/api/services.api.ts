...
type ServiceWrite = { name: string; description: string; price: number; sessions: number; status: boolean; imageFile?: File; removeImage?: boolean };
function serviceFormData(data: ServiceWrite) { const form = new FormData(); form.append("name", data.name); form.append("description", data.description); form.append("price", String(data.price)); form.append("sessions", String(data.sessions)); form.append("status", String(data.status)); if (data.imageFile) form.append("image", data.imageFile); if (data.removeImage) form.append("removeImage", "true"); return form; }
